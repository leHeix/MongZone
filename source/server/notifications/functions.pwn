...
Float:EaseInOutBack(Float:x)
{
	...
	return x < 0.5
	? (floatpower(2 * x, 2) * ((c2 + 1) * 2 * x - c2)) / 2
	: (floatpower(2 * x - 2, 2) * ((c2 + 1) * (x * 2 - 2) + c2) + 2) / 2;
}

Float:lerp(Float:p1, Float:p2, Float:t)
	return p1 + (p2 - p1) * t;

Notification_Show(playerid, const text[], time = 2000)
{
	new td_idx = Iter_Free(g_rgNotifUsed[playerid]);
	if(td_idx == INVALID_ITERATOR_SLOT)
	{
		print("invalid iter slot");
		...
		strcpy(item[e_szNotificationText], text);
	...
	PlayerTextDrawSetString(playerid, p_tdNotifications[playerid][td_idx]{4}, text);
