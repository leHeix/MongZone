#if defined _PLAYER_INTRODUCTION_FUNCTIONS_
	#endinput
#endif
#define _PLAYER_INTRODUCTION_FUNCTIONS_

/* 
	what a mess! can't rely on pawnplus to work flawlessly on production
	overloading the server with wait_ms may (and will) eventually fail
...
public __Intro_TenthWait(playerid)
{
	...
	FCNPC_OnPlaybackEndInline(g_rgeIntroData[playerid][e_iTaxiDriver], using inline PlaybackEnd);
...
public __Intro_FourteenthWait(playerid)
{
	...
	SetSpawnInfo(playerid, NO_TEAM, Player_GetSkin(playerid), 1483.0563, -1738.4750, 13.5469, 358.7972, 0, 0, 0, 0, 0, 0);
...
public __Intro_FifteenthWait(playerid)
{
	...
	Bit_Set(Player_GetFlags(playerid), PFLAG_INTRODUCTION_RUNNING, false);
...
Intro_ToggleTextdraws(playerid, bool:show)
{
	if(show)
	{
		for (new i = (sizeof(g_tdRegisterAcc) - 1); i != -1; i -= 1)
		{
